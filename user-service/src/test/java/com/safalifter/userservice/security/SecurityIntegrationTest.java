package com.safalifter.userservice.security;
...
class SecurityIntegrationTest {
    ...
    @Test
    @DisplayName("ADMIN 访问 admin 接口 (getAll) 应该返回 200 OK")
    ...
    @Test
    @DisplayName("USER 访问 admin 接口 (getAll) 应该返回 403 Forbidden")
    void userAccessAdminEndpoint_shouldReturn403() throws Exception {
    ...
    @Test
    @DisplayName("无 Token 访问 admin 接口 应该返回 403 Forbidden")
    void noTokenAccessAdminEndpoint_shouldReturn403() throws Exception {
        mockMvc.perform(get("/v1/user/getAll")
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isForbidden());
    }
    ...
    @Test
    @DisplayName("格式错误的 Authorization header (无 Bearer) 应该继续到 Spring Security 返回 403")
    void malformedAuthHeader_shouldReturn403() throws Exception {
        mockMvc.perform(get("/v1/user/getAll")
    ...
    @Test
    @DisplayName("没有 Bearer 前缀的 Token 应该继续到 Spring Security 返回 403")
    void tokenWithoutBearerPrefix_shouldReturn403() throws Exception {
        String token = createToken("admin", "ROLE_ADMIN", 3600000);
    ...
    @Test
    @DisplayName("ADMIN 访问公共接口 (getUserById) 应该返回 200 OK")
    void adminAccessPublicEndpoint_shouldSucceed() throws Exception {
        String adminToken = createToken("admin", "ROLE_ADMIN", 3600000);

        when(userService.getUserById("123")).thenReturn(null);

        mockMvc.perform(get("/v1/user/getUserById/123")
                        .header("Authorization", "Bearer " + adminToken)
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk());
    }
    ...
    @Test
    @DisplayName("USER 访问公共接口 (getUserById) 应该返回 200 OK")
